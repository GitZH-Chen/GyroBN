...
class RBNH(nn.Module):
    def __init__(self, dim, manifold,momentum=0.1):
        super(__class__, self).__init__()
        self.man = manifold;self.dim=dim;self.momentum=momentum
        ...
        if self.training:
            ...
            if self.running_mean is None:
                ...
                self.running_mean = self.man.exp(
                    self.running_mean,
                    self.momentum * self.man.log(self.running_mean, input_mean)
                )
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(dim={self.dim}, manifold={self.man}, momentum={self.momentum})"
